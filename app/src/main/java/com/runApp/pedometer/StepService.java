...

{
    ...
    public void onCreate() {
        LogUtils.LOGE(TAG, "[SERVICE] onCreate");
        super.onCreate();
...
//        mUtils.initTTS();
        ...
        // Start detecting
//        mStepDetecterWithAPI = new StepDetecterWithAPI();
...
//        reloadSettings();
    ...
    public void onDestroy() {
        LogUtils.LOGE(TAG, "[SERVICE] onDestroy");
//        mUtils.shutdownTTS();
...
//        mStateEditor.commit();
    ...
    private void registerDetector() {
        LogUtils.LOGE(TAG, "[SERVICE] registerDetector");
    ...
    private void unregisterDetector() {
        LogUtils.LOGE(TAG, "[SERVICE] unregisterDetector");
        mSensorManager.unregisterListener(mStepDetector);
    }

    @Override
    public IBinder onBind(Intent intent) {
        LogUtils.LOGE(TAG, "[SERVICE] onBind");
        return mBinder;
    }
    ...
    public void reloadSettings() {
        mSettings = PreferenceManager.getDefaultSharedPreferences(this);
        LogUtils.LOGE(TAG, "reloadSettings " + mSettings.getInt("steps", 0));
    ...
    private StepDisplayer.Listener mStepListener = new StepDisplayer.Listener() {
        public void stepsChanged(int value) {
            if (value < UserUtils.getStepsNumber()) {
                LogUtils.LOGE(TAG, "reloadSettings " + UserUtils.getStepsNumber());
                value = UserUtils.getStepsNumber() + 1;
                mStepDisplayer.setSteps(value);
            }

            if (value == 5 || value == 20 || value == 10 || value == 15) {
//                showNotification(value);
    ...
    private void showNotification(int value) {
        ...
        notification.setLatestEventInfo(this, text,
                (getText(R.string.notification_subtitle) + " @ " + value), contentIntent);
